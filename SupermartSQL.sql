...
join(select * from product) as b on a.product_id=b.product_id
join(select * from customer) as c on a.customer_id=c.customer_id

/* To analyze and understand the topic of Joins, a new table was created*/
/* Creating a sales table of 2015*/
create table sales_2015 as 
select * from sales
where shipdate between '2015-01-01' and '2015-12-31'

/* Creating a customer table with age between 20 and 60*/
...
/* Task: Find the total  sales done in every state for customer_20_60 and sales_2015 table*/
/* For some reason, this code is error.*/
select
...
/* Task: get data containing Product_ID, Product_Name, Category, Total_Sales value of that product  and total quantity sold*/
select * 
from product
...
/* I have a habit of experimenting and testing out the syntax to be further familiar*/
/* Thus, I show correct and incorrect syntax*/
...
/* Exercise on Views */
/* Task: Create a view which contains order_line, Product_id, sales
and discount value of the first order date in the sales table and name
it as "Daily Billing"*/
select *
from sales
...
/*Task: 1. Find Maximum length of characters in the product name string
...
/*Task: 2. Retrieve product name, sub-category and category from product
table and an additional column named "product_details" which contains a
concatenatred string product name, sub_categroy and category*/
...
/*Task: 3. Analyze the product_id column and take out the three
parts composing the product_id in three different columns*/
...
/*Task: 4. List down comma sperated product name where sub_category
is ether chairs or tables*/
...
/* Exercise 14  */
/*Task 1. You are running a lottery for your customers. So, Pick
...
order by random()
limit 5
/*Syntax above are ways to use the random function for the purpose of the task*/

/*Task 2. Suppose you cannot charge the custoimer in fraciton points. So,
for sales value of 1.63, you will get either 1 or 2. In such scenario, find out
a) Total sales revenue if you are charging the lower integer of sales always
b) Total sales revenue if you are charging the higher integer of sales always
...
--2. Analyze and find out monthly sales and sub-category chair
select * from product
...
/*Task 1. Find out all customer who have first name and last name of 5 
characters each and last name starts with "a/b/c/d"*/
...
/*task: 2 Create a table "zipcode" and insert the below the data
. Find out the valid zipcodes from this talbe*/
